...
ln -sfn "$RUN_ROOT" /vepfs/tsra_outputs/formal_10ep/latest_prop_deberta_seed1_gpu7_retry
...
run_task() {
  ...
  if [[ "$rc" -eq 0 ]]; then
    ...
    { cat "$started"; echo "failed_at=$(date '+%F %T')"; echo "exit_code=$rc"; tail -n 120 "$log"; } > "$failed"
    echo "[$(date '+%F %T')] <<< $name failed rc=$rc" | tee -a "$RUN_ROOT/queue.log"
  fi
}

run_task proofwriter_deberta_baseline_seed1_10ep \
  --dataset proofwriter \
...
run_task proofwriter_deberta_tsra_seed1_10ep \
  --dataset proofwriter \
  --root data/proofwriter/raw/proofwriter-dataset-V2020.12.3 \
  --train-depths 0,1,2 \
  --test-depths 3,5 \
...
run_task ruletaker_deberta_baseline_seed1_10ep \
...
run_task ruletaker_deberta_tsra_seed1_10ep \
  --dataset ruletaker_gfair \
  --root external_baselines/GFaiR/data/ruletaker_3ext_sat \
  --limit-train 0 \
  --limit-test 0 \
...
run_task prontoqa_deberta_baseline_seed1_10ep \
  --dataset prontoqa \
...
run_task prontoqa_deberta_tsra_seed1_10ep \
  --dataset prontoqa \
  --root data/prontoqa_ood/processed/generated_ood_data \
  --limit-train 0 \
  --limit-test 0 \
...
echo "[$(date '+%F %T')] prop deberta seed1 retry queue finished" | tee -a "$RUN_ROOT/queue.log"
