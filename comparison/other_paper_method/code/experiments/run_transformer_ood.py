...
class CLUTRRDataset(Dataset):
  ...
  def __getitem__(self, i):
    ...
    try:
        story_edges = safe_literal_eval(self.data[i][11], default=None)  # L列
        edge_types = safe_literal_eval(self.data[i][12], default=None)   # M列
        node_mapping = safe_literal_eval(self.data[i][16], default=None) # Q列
        if story_edges is None or edge_types is None or node_mapping is None:
